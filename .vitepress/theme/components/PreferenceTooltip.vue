  ...
  <Transition name="fly-in">
    <div class="preference-tooltip" v-if="show">
      <template v-if="source === 'default'">
        <p>Стиль по замовчуванню: Композиційний API.</p>
        <p>
          Деякі сторінки містять різний вміст залежно від обраного стилю API.
         Використовуйте цей перемикач для перемикання між стилями API.
        </p>
      </template>
      <template v-if="source && source.startsWith('url')">
        <p>
          Показано вміст для
          {{ preferComposition ? 'Композиційного' : 'Опційного' }} API, оскільки
          {
          {{
            source === 'url-query'
              ? 'він визначається запитом URL.'
              : 'цільовий розділ доступний лише для цього API.'
          }}
        </p>
        <p>
          Це відрізняється від ваших збережених налаштувань і вплине лише на
          поточний сеанс перегляду.
        </p>
      </template>
      <p class="actions">
        <a href="/guide/introduction#api-styles">Дізнатися більше</a>
        <button @click="dismiss">Зрозуміло</button>
      </p>
