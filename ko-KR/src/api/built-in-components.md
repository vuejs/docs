...
- **사용방법:**
    ...
    동적 컴포넌트를 렌더링 하기위한 "메타 컴포넌트" 입니다. 렌더링 할 실제 컴포넌트는 `is` prop 에 의해 결정됩니다. 문자열로서의 <code>is</code> prop 은 HTML 태그 이름 또는 컴포넌트 이름 일 수 있습니다.
...
- **Props:**
    ...
    - `appear` - `boolean`, Whether to apply transition on initial render. 기본값  `false`.
    - `persisted` - `boolean`. true 이면, <br>실제로 요소를 삽입(insert)/삭제(remove)하지 않고 대신에 표시(show)/숨김(hidden) 상태를 전환하는 transition 임을 나타냅니다. transition hooks 가 삽입되지만, 렌더러에서 건너뜁니다. 대신, 사용자 디렉티브에 삽입된 hooks 는 호출하여 transition 을 조작할 수 있습니다.(예. <code>v-show</code>)
...
- **사용방법:**
    ...
    애니메이션이 제대로 동작을 하려면, <code><transition-group></code> 컴포넌트의 모든 자식요소에 <a><strong>고유한 키 지정(uniquely keyed)</strong></a>이 되어야 한다는점에 주의하세요.
