...
### Сигнатура функції <sup class="vt-badge" data-text="3.3+" /> {#function-signature}

`defineComponent()` також має альтернативну сигнатуру, яка призначена для використання з композиційним API та [функціями рендерингу або JSX](/guide/extras/render-function.html).

Замість передачі об'єкта параметрів очікується функція. Ця функція працює так само, як функція композиційного API [`setup()`](/api/composition-api-setup.html#composition-api-setup): вона отримує атрибути та контекст налаштування. Значення, яке повертається, має бути функцією рендерингу - підтримуються як `h()`, так і JSX:
