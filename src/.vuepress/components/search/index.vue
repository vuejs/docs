<template>
  <main id="search-page" class="DocSearch">
    ...
    <p v-if="!isAlgoliaConfigured">
      Ця сторінка пошуку наразі недоступна, будь ласка, скористайтеся вікном пошуку у верхній панелі навігації.
    </p>
    ...
    <template v-else>
      ...
      <div class="search-footer">
        ...
        <p>
          <template v-if="totalResults">
            <strong>Знайдено {{ totalResults }} результатів</strong> за {{ queryTime }}мс
          </template>
        </p>

        <div class="DocSearch-Logo">
          <a href="https://www.algolia.com/docsearch" target="_blank" rel="noopener noreferrer">
            <span class="DocSearch-Label">Шукати як</span>
      ...
      <p v-else-if="search">Відсутні результати для запиту "<span v-text="search" />".</p>
