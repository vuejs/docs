...
`toValue()` — API, доданий у версії 3.3. Він призначений для нормалізації посилань або геттерів у значення. Якщо аргумент є посиланням, він повертає значення посилання; якщо аргумент є функцією, він її викличе та поверне її значення. В іншому випадку він повертає аргумент як є. Він працює подібно до [`unref()`](/api/reactivity-utilities.html#unref), але з особливою обробкою функцій.
