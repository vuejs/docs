...
За допомогою Опційного API ми можемо використовувати опцію [`watch`](/api/options-state.html#watch), щоб запускати функцію щоразу, коли змінюється реактивна властивість:
...
### Вихідні типи спостерігачів  {#base-example}
...
`watch` за замовчуванням є лінивим: зворотний виклик не буде викликано, доки не зміниться спостережуване джерело. Але в деяких випадках ми можемо захотіти, щоб та сама логіка зворотного виклику запускалася невідкладно - наприклад, ми можемо захотіти отримати деякі початкові дані, а потім повторно отримати дані щоразу, коли відповідний стан змінюється. Ми можемо це робити так:
...
// витягнути негайно
fetchData()
// ...тоді спостерігати за зміною URL-адреси
watch(url, fetchData)
```
...
Тут зворотний виклик запуститься негайно. Під час виконання він також автоматично відстежуватиме `url.value` як залежність (подібно до обчислюваних властивостей). Щоразу, коли `url.value` змінюється, зворотний виклик запускатиметься знову.
...
## Час спрацювання зворотного виклику {#callback-flush-timing}
...
Спостерігачі, оголошені за допомогою опції `watch` або методу екземпляра `$watch()`, автоматично зупиняються, коли компонент власника демонтується, тому в більшості випадків вам не потрібно турбуватися про зупинку спостерігача самостійно.
