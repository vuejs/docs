...
Спостерігачі, оголошені за допомогою параметра `watch` або методу екземпляра `$watch()`, автоматично зупиняються, коли компонент власника демонтується, тому в більшості випадків вам не потрібно самостійно турбуватися про зупинку спостерігача.
