...
Якщо Ви добре знайомі з концепціями Virtual DOM і віддаєте перевагу необробленій потужності JavaScript, Ви також можете [писати рендер-функції](/guide/extras/render-function) замість шаблонів, з додатковою підтримкою JSX. Проте, не забувайте, що вони не є такого ж рівня оптимізації під час компіляції, як шаблони.
...
Тег "вуса" буде замінено на значення властивості `msg` з [відповідного екземпляра компонента](/guide/essentials/reactivity-fundamentals#declaring-reactive-state). Він також буде змінюватись кожного разу, коли властивість `msg` змінюватиметься.

## Чистий HTML {#raw-html}

Подвійні хвилясті дужки інтерпретують дані як звичайний текст, а не HTML. Щоб вивести справжній HTML, вам потрібно буде використовувати [директиву `v-html`](/api/built-in-directives#v-html):
...
"Вуса" не можуть використовуватись всередині HTML атрибутів. Натомість використовуйте [директиву `v-bind`](/api/built-in-directives#v-bind):
...
Вирази шаблонів є ізольованим програмним середовищем і мають доступ лише до [списку обмежених глобальних елементів](https://github.com/vuejs/core/blob/main/packages/shared/src/globalsAllowList.ts#L3). У списку представлені вбудовані глобальні значення, які часто використовуються, як-от `Math` і `Date`.
...
Директиви — це спеціальні атрибути з префіксом `v-`. Vue надає ряд [вбудованих директив](/api/built-in-directives), включаючи `v-html` і `v-bind`, які ми представили вище.
...
Якщо вам потрібно передати складний динамічний аргумент, можливо, краще використовувати [обчислену властивість](./computed), яку ми розглянемо незабаром.
...
Пізніше ви побачите інші приклади модифікаторів [для `v-on`](./event-handling#event-modifiers) та [для `v-model`](./forms#modifiers), коли ми досліджуватимемо ці функції.
