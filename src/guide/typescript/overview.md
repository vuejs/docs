...
Система типів, така як TypeScript, може виявляти багато помилок за допомогою статичного аналізу під час збірки. Це зменшує ймовірність появи помилок під час виконання у production, а також дозволяє більш впевнено рефакторювати код у великомасштабних проектах. TypeScript також покращує ергономіку розробника завдяки автоматичному заповненню типів у IDE.

Vue написаний на TypeScript забезпечує першокласну підтримку TypeScript. Усі офіційні пакети Vue постачаються в комплекті з деклараціями типів, які повинні працювати з коробки.
...
З базовими налаштуваннями Vite, сервер розробки і комплектувальник використовуються лише для транспіляції, та не виконують жодної перевірки типів. Це забезпечує блискавичну роботу сервера розробників Vite навіть при використанні TypeScript.

- Під час розробки рекомендуємо покладатися на хорошу [IDE підтримку](#ide-підтримка) для миттєвого зворотнього зв'язку при помилках типу.

- Якщо ви використовуєте одно-файлові компоненти, використовуйте [`vue-tsc`](https://github.com/johnsoncodehk/volar/tree/master/packages/vue-tsc) утиліту для перевірки типу командного рядка та створення декларації типу. `vue-tsc` це обгортка навколо `tsc`, власного інтерфейсу командного рядка TypeScript. Він працює майже так само, як `tsc`, за винятком того, що він підтримує Vue одно-файлові компоненти на додаток до файлів TypeScript. Ви можете запустити `vue-tsc` у режимі перегляду, паралельно із сервером розробки Vite або використати плагін Vite, наприклад [vite-plugin-checker](https://vite-plugin-checker.netlify.app/) який запускає перевірку в окремому робочому потоці.

- Vue CLI також забезпечує підтримку TypeScript, але більше не рекомендується. Дивіться [примітки нижче](#примітка-щодо-vue-cli-та-ts-loader).

### IDE Підтримка

- [Visual Studio Code](https://code.visualstudio.com/) (VSCode) настійно рекомендується через чудову готову підтримку TypeScript.

  - [Volar](https://marketplace.visualstudio.com/items?itemName=Vue.volar) є офіційним розширенням VSCode, яке забезпечує підтримку TypeScript в одно-файлових компонентах Vue, а також багато інших чудових функцій.
...
Проекти, створені за допомогою `create-vue`, містять попередньо налаштований `tsconfig.json`. Базова конфігурація абстрагується у [`@vue/tsconfig`](https://github.com/vuejs/tsconfig) пакеті. Всередині проекту ми використовуємо [Project References](https://www.typescriptlang.org/docs/handbook/project-references.html) щоб забезпечити правильні типи для коду, що виконується в різних середовищах (наприклад, код програми та код тесту повинні мати різні глобальні змінні).
...
- Якщо ви налаштували псевдоніми розпізнавателя в інструменті збірки, наприклад, псевдонім `@/*`, що налаштований за замовчуванням у проекті `create-vue`, вам також потрібно налаштувати його для TypeScript через [`compilerOptions.paths`](https://www.typescriptlang.org/tsconfig#paths).
...
Щоб забезпечити спільну роботу Vue одно-файлових компонент і TypeScript, Volar створює окремий екземпляр мовної служби TS із підтримкою Vue і використовує його в Vue одно-файлових компонентах. У той же час звичайні файли TS все ще обробляються вбудованою мовною службою VSCode, тому нам потрібен [TypeScript Vue Плагін](https://marketplace.visualstudio.com/items?itemName=Vue.vscode-typescript-vue-plugin) для підтримки імпорту Vue одно-файлових компонент у файли TS. Це налаштування працює за замовчуванням, але для кожного проекту ми запускаємо два екземпляри мовної служби TS: одну від Volar і одну від вбудованої служби VSCode. Це трохи неефективно та може призвести до проблем із продуктивністю у великих проектах.

Для покращення продуктивності, Volar надає функцію під назвою «Режим Takeover». Цей режим забезпечує підтримку як файлів Vue, так і файлів TS, за допомогою єдиного екземпляру мовної служби TS.

Щоб увімкнути режим Takeover, вам потрібно вимкнути вбудовану службу мови TS VSCode **лише у робочій області вашого проекту**, виконавши такі дії:

1. У робочій області проекту відкрийте панель команд за допомогою `Ctrl + Shift + P` (macOS: `Cmd + Shift + P`).
...
- [Типові випробування для `defineComponent`](https://github.com/vuejs/core/blob/main/test-dts/defineComponent.test-d.tsx)
...
:::tip
У разі використання Vue CLI, або налаштування на основі веб-пакета, TypeScript вимагає у виразах шаблону `vue-loader@^16.8.0`.
:::

## API-спеціальні рецепти
