...
"Прохідний атрибут" — це атрибут або `v-on` слухач подій, який передається компоненту, але не декларується явно в [реквізитах](./props) або [випромінюваннях](./events.html#declaring-emitted-events) приймаючого компонента.. Типовими прикладами цього є атрибути `class`, `style` та `id`.
