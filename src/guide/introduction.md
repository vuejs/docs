...
За допомогою Options API ми визначаємо логіку компонента за допомогою об'єкта параметрів таких як `data`, `methods` і `mounted`. Властивості, визначені параметрами, виділяються у внутрішні функції `this`, що своєю чергою вказує на екземпляр компонента:
...
Обидва стилі API цілком здатні охопити стандартні випадки використання. Це різні інтерфейси, які працюють від однієї й тієї ж основної системи. Фактично, Options API реалізовано поверх Composition API! Основні концепції та знання про Vue спільні для обох стилів.
