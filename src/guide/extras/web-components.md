...
Vue підтримує створення користувацьких елементів за допомогою тих самих API компонентів Vue за допомогою методу [`defineCustomElement`](/api/general#definecustomelement). Метод приймає той самий аргумент, що й [`defineComponent`](/api/general#definecomponent), але натомість повертає власний конструктор елемента, який розширює `HTMLElement`:
...
- [Обмежені слоти](/guide/components/slots#scoped-slots) не підтримуються.
