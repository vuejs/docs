...
При надходженні повідомлення про вразливість, воно негайно стає нашим головним занепокоєнням, оскільки постійний учасник кидає все і займається ним. Щоб повідомити про вразливість, будь ласка, надішліть повідомлення на електронну пошту [security@vuejs.org](mailto:security@vuejs.org).
...
Основне правило безпеки під час використання Vue — **ніколи не використовуйте ненадійний вміст, як шаблон компонента**. Це еквівалентно дозволу довільного виконання JavaScript у вашій програмі – і, що ще гірше, може призвести до зламу сервера, якщо код виконується під час рендерингу на стороні сервера. Приклад такого використання:
...
Таким чином запобігаючи впровадженню шкідливого скрипту. Екранування виконується за допомогою нативного API браузера, такого як `textContent`, тому вразливість можлива лише у випадку, якщо сам браузер вразливий.
...
У будь-якому веб-додатку можливість виконання вмісту користувача без санітизації у форматі HTML, CSS, або JavaScript є потенційно небезпечним, і цього слід уникати скрізь де тільки можливо. Але бувають моменти, коли певний ризик може бути прийнятним.
...
Існує потенційна проблема безпеки, якщо URL-адреса не була "санітизована" для попереднього виконання JavaScript через `javascript:`. Є бібліотеки, такі як [sanitize-url](https://www.npmjs.com/package/@braintree/sanitize-url) які можуть з цим допомогти, але зверніть увагу: якщо ви колись займалися санітизацією URL на фронтенді, то проблема з безпекою вже є. **URL-адреси, що надаються користувачем, завжди повинні санітизуватися на бекенді, перед збереженням до бази даних.** Тоді проблема буде вирішена для _кожного_ клієнта, який підключається до вашого API, у тому числі нативні мобільні додатки. Ще слід пам'ятати, що навіть з санітизованими URL, Vue не гарантує вам, що вони ведуть на безпечні ресурси.
...
Припустимо, що `sanitizedUrl` був санітизований і це дійсно справжній URL, а не JavaScript. Але використовуючи `userProvidedStyles`, зловмисники досі можуть додати CSS для "click jack", тобто. стилізувати посилання прозорим блоком, наприклад, поверх кнопки «Входу в систему». У такому разі, якщо `https://user-controlled-website.com/` створено так, що, візуально повторює сторінку авторизації програми, тоді з'являється можливість перехоплення логінів і паролів користувачів.

Можливо, ви можете собі уявити, як дозволений наданий користувачем вміст для елемента `<style>` може створити ще більшу вразливість, надаючи користувачеві повний контроль над тим, як стилізувати всю сторінку. Ось чому Vue запобігає рендерити теги стилю всередині шаблонів, наприклад:

```vue-html
<style>{{ userProvidedStyles }}</style>
```

Щоб повністю захистити своїх користувачів від click jacking, ми рекомендуємо надавати повний контроль над CSS лише всередині iframe ізольованого програмного середовища. Крім того, якщо користувачеві надається контроль за допомогою прив’язки стилю, ми рекомендуємо використовувати його [об'єктний синтаксис](/guide/essentials/class-and-style.html#binding-to-objects-1) і дозволяти користувачеві вказувати лише значення для конкретних властивостей, які можна змінювати безпечно, наприклад:
...
### JavaScript Впровадження

Настійно не рекомендуємо рендерити елементи `<script>` за допомогою Vue, оскільки шаблони та функції рендерингу ніколи не повинні мати побічних ефектів. Однак це не єдиний спосіб включити рядки, які будуть оцінюватися як JavaScript під час виконання.
...
1. Розробник явно вказує Vue рендерити наданий користувачем не санітизований вміст у шаблонах Vue. Це за своєю суттю небезпечно, і Vue не має можливості відстежувати це.
...
Загальне правило полягає в тому, що якщо ви дозволяєте виконувати не санітизований, наданий користувачами вміст (як HTML, JavaScript або навіть CSS), ви можете бути відкритими для атак. Ця порада справді актуальна незалежно від того, чи використовується Vue, інший фреймворк чи навіть без фреймворку.

Крім вищевикладених рекомендацій з розділу [Потенційних небезпек](#potential-dangers), рекомендуємо також ознайомитись з такими ресурсами:
