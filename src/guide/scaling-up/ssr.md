...
Якщо ви досліджуєте SSR лише для покращення SEO кількох маркетингових сторінок (наприклад, `/`, `/about`, `/contact` тощо), тоді вам, мабуть, потрібен SSG замість SSR. SSG також чудово підходить для веб-сайтів із вмістом, таких як сайти документації чи блоги. Фактично, цей веб-сайт, який ви зараз читаєте, статично згенерований за допомогою [VitePress](https://vitepress.dev/), генератора статичних сайтів на основі Vue.
...
[`renderToString()`](/api/ssr#rendertostring) приймає екземпляр застосунку Vue та повертає Promise, який надає рендериний HTML застосунку. Також можна транслювати рендеринг за допомогою [Node.js Stream API](https://nodejs.org/api/stream.html) або [Web Streams API](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API). Для повної інформації перегляньте [SSR API Reference](/api/ssr).
...
Щоб змонтувати застосунок в режимі гідрації, нам потрібно використовувати [`createSSRApp()`](/api/application#createssrapp) замість `createApp()`:
...
У розділі "керування станом" ми надали [простий шаблон керування станом за допомогою API реактивності](state-management#simple-state-management-with-reactivity-api). У контексті SSR цей шаблон вимагає деяких додаткових коригувань.
...
Рекомендованим рішенням є створення нового екземпляра всього застосунку, включаючи маршрутизатор і глобальні сховища, за кожним запитом. Тоді, замість того, щоб безпосередньо імпортувати його в наші компоненти, ми надаємо спільний стан за допомогою [надавання рівня застосунку](/guide/components/provide-inject#app-level-provide) і вводимо його в компоненти, яким це потрібно:
