...
Вам не потрібно нічого встановлювати на свій комп’ютер, щоб випробувати одно-файлові компоненти Vue – існують онлайн-пісочниці, які дозволяють робити це прямо у браузері:
...
Команда Vue підтримує [eslint-plugin-vue](https://github.com/vuejs/eslint-plugin-vue), плагін [ESLint](https://eslint.org/), який підтримує спеціальне лінтування одно-файлових компонентів.

Користувачі, які раніше використовували Vue CLI, можуть звикнути до конфігурації лінтерів за допомогою webpack. Однак, якщо використовується налаштування збірки на основі Vite, наша загальна рекомендація:
...
2. Налаштуйте розширення IDE ESLint, наприклад [ESLint для VSCode](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint), щоб ви отримували повідомлення від лінтера прямо у своєму редакторі під час розробки. Це також дозволяє уникнути зайвих витрат на перевірку під час запуску сервер розробки.

3. Запустіть ESLint як частину команди виробничої збірки, щоб отримати повний відгук лінтера перед публікацією.
...
- Розширення VSCode [Volar](https://github.com/johnsoncodehk/volar) одразу забезпечує форматування для одно-файлових компонентів Vue.

- Крім того, [Prettier](https://prettier.io/) забезпечує вбудовану підтримку форматування одно-файлових компонентів Vue.
...
Сам пакет надає утиліти нижчого рівня для обробки одно-файлових компонентів Vue і призначений лише для авторів інструментів, яким потрібно підтримувати одно-файлові компоненти Vue у користувацьких інструментах.
...
Офіційний плагін, який забезпечує підтримку одно-файлових компонентів Vue у Vite.

### `vue-loader`

- [Документація](https://vue-loader.vuejs.org/)

Офіційний завантажувач, який забезпечує підтримку одно-файлових компонентів Vue у webpack. Якщо ви використовуєте Vue CLI, також перегляньте [документацію про зміну параметрів `vue-loader` у Vue CLI](https://cli.vuejs.org/guide/webpack.html#modifying-options-of-a-loader) .
